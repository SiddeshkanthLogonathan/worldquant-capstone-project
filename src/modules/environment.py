...
class MultiPeriodPortfolioOptimizationEnv(gym.Env):
    ...
    def __init__(
        ...
        self._multi_period_horizon = multi_period_horizon
        ...
        # results file
        self._results_file = self._cwd/"results"/"rl"/experiment_type
    ...
    def step(self, actions):
        ...
        """
        self._terminal = self._time_index >= len(self._sorted_times) - self._multi_period_horizon
        ...
        if self._terminal:
            ...
            plt.savefig(self._results_file / f"{self.suffix}_portfolio_value.png")
            plt.close()
            ...
            plt.xlabel("Time")
            plt.ylabel("Reward")
            plt.savefig(self._results_file / f"{self.suffix}_reward.png")
            plt.close()

            plt.plot([x[0, :] for x in self._actions_memory[1:]])
            plt.title("Actions performed")
            plt.xlabel("Time")
            plt.ylabel("Weight")
            plt.savefig(self._results_file / f"{self.suffix}_actions.png")
            plt.close()
            ...
            qs.plots.snapshot(
                metrics_df["returns"],
                show=False,
                savefig=self._results_file / f"{self.suffix}_portfolio_summary.png",
            )
            ...
            elif self._comission_fee_model == "trf": # WE'RE USING THIS METHOD FOR NOW......
                last_mu = np.ones(self._multi_period_horizon)
                mu = np.repeat(1 - 2 * self._comission_fee_pct + self._comission_fee_pct**2, repeats=self._multi_period_horizon)
                while np.any(np.abs(mu - last_mu)) > 1e-10: # MAY INDUCE A NUMERICAL INSTABILITY
                    last_mu = mu
                    mu = (
                        1
                        - self._comission_fee_pct * weights[:, 0] # Cash
                        - (2 * self._comission_fee_pct - self._comission_fee_pct**2)
                        * np.sum(np.maximum(last_weights[:, 1:] - mu[:, None] * weights[:, 1:], 0), 1)
            ...
            portfolio = mu[:, None] * weights * self._price_variation
            # calculate new portfolio value and weights
            # self._portfolio_value = np.sum(portfolio, 1)
            weights = portfolio / np.sum(portfolio, 1)[:, None]
            self._portfolio_value = self._portfolio_value*portfolio.sum(axis=1).prod()
            # save final portfolio value and weights of this time step
            self._asset_memory["final"].append(self._portfolio_value) # MULTI-PERIOD TIME REDUCTION
    ...
    def _get_state_and_info_from_time_index(self, time_index):
        ...
        end_time_state = self._sorted_times[time_index]
        start_time = self._sorted_times[time_index - (self._time_window - 1)]
        time_multi_period = self._sorted_times[time_index:time_index+self._multi_period_horizon]
        # define data to be used in this time step
        self._data = self._df[
            (self._df[self._time_column] >= start_time)
            & (self._df[self._time_column] <= end_time_state)
        ...
        #     self._df_price_variation[self._time_column] == end_time_state
        # ][self._valuation_feature].to_numpy()
        # self._price_variation = np.insert(self._price_variation, 0, 1)
        # self._price_variation = self._price_variation[None, :]

        self._price_variation = self._df_price_variation[self._df_price_variation[self._time_column].isin(time_multi_period)]
        self._price_variation = self._price_variation.sort_values([self._time_column, self._tic_column])[[self._time_column, self._tic_column, self._valuation_feature]]
        self._price_variation = self._price_variation.pivot(index=self._time_column, columns=self._tic_column, values=self._valuation_feature).to_numpy()
        self._price_variation = np.insert(self._price_variation, 0, 1, axis=1) # Return on Cash filled with 1
    ...
    def _softmax_normalization(self, actions):
        ...
        normalized_action_vector = softmax(actions, axis=1)
    ...
    def _reset_memory(self):
        ...
        self._actions_memory = [
            np.tile(np.array([1]+[0]*self.portfolio_size, dtype=np.float32), reps=(self._multi_period_horizon,1))
        ]
